...
module Jooxe
  ...
  describe Jooxe::Controller do
    ...
    it "should show nothing when calling index" do
      ...
      #view.collection.size.should eq(3)
    end
    
    it "should retrieve a collection when calling list" do
      setup_URI "http://localhost:9292/users/list"
      options = @router.route(@env)
      options[:model_class_name].should eq('User')
      options[:table_name].should eq('users')
      options[:controller_class].should be_an_instance_of(UsersController)
      options[:action].should eq('list')
